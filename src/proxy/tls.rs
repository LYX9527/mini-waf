...
/// 启动 HTTPS/TLS 网关
pub async fn start_tls_proxy_server(state: Arc<AppState>, cert_path: &str, key_path: &str, port: u16) {
    crate::log_info!("TLS_INIT", "加载 SSL 证书: {}", cert_path);
    let certs = load_certs(cert_path).expect("加载证书失败");
    let key = load_private_key(key_path).expect("加载私钥失败");
    ...
    // 支持 ALPN，声明支持 HTTP/1.1 (如果需要 h2，则需配置 HTTP/2)
    ...
    crate::log_success!("HTTPS_PROXY", "HTTPS 反向代理已启动，监听 {}", addr);
